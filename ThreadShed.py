...
for x in daily_transactions_split:
    eachClean = []
    for y in x:
        eachClean.append(y.replace('\n', '').strip(' '))
        transactions_clean.append(eachClean)
...
def color_count(color):
    color_total = 0
    for eachThreadColor in thread_sold_split:
...
for color in colors:
    print("Thread Shead Sold {0} threads of {1} thread today.".format(color_count(color), color))
